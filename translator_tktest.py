...
msg = 'hey send_tweet hey dudes and dudettes listening to ween doing some twitch'
...
def shuffle_text():
    ...
    for i in msg.split(' '):
        for j in i.split():
            slow_print.append(j)
            label.config(text=' '.join(slow_print))
            time.sleep(1)
    # window.after(100, shuffle_text)

window = tk.Tk()
window.wm_overrideredirect(False)
window.geometry('780x360')
window.bind("<Button-1>", lambda evt: window.destroy())
window.configure(background='white')

label = tk.Label(text='', font=("Helvetica", 17))
